"""Panel V2.5.1 de señales LONG/SHORT independientes de las posiciones abiertas."""
...
def _dibujar_periodo(
    ...
    for evento in eventos_calendario or []:
        ...
        grosor = 2.0 if es_resultado else 1.3
...
def generar_panel_senales(
    ...
    *,
    ...
    tarjetas = []
    if mostrar:
        print(f"PANEL DE SEÑALES V2.5.1 — cierre {ultima_fecha.date()}")
    for ticker in tickers:
        tabla = datos[ticker]
        fin = tabla.index[-1]
        ...
        ruta_grafico = salida / f"{ticker}_panel_v2_5_1.png"
...
<title>Panel de señales V2.5.1</title></head><body style='font-family:Arial;max-width:1500px;margin:auto'>
<h2>Panel de señales V2.5.1</h2><p>Cierre analizado: <b>{ultima_fecha.date()}</b></p>
{''.join(tarjetas)}
...
{ventana_senales_dias} días y no transmite órdenes.</p>
    ...
    archivo_html = salida / "resumen_senales_v2_5_1.html"
    archivo_html.write_text(resumen, encoding="utf-8")
    if mostrar:
        print("Resumen V2.5.1 guardado en:", archivo_html.resolve())
    return {
        "tickers": tickers,
        "datos": datos,
        "senales": estados,
