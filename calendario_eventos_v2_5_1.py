"""Calendario vertical de catalizadores para el cuaderno operativo V2.5.1.
...
MESES_HISTORICOS = 6
MESES_PROXIMOS = 2
UMBRAL_MOVIMIENTO_PCT = 5.0
UMBRAL_MOVIMIENTO_FUERTE_PCT = 7.0

CARPETA_CALENDARIO = Path("salida_operativa") / "calendario_resultados"
ARCHIVO_ESTADO = CARPETA_CALENDARIO / "estado_fechas_v2_5_1.json"
ARCHIVO_HTML = CARPETA_CALENDARIO / "informe_resultados_v2_5_1.html"
...
def _descargar(url: str, timeout: int = 25) -> tuple[bytes, str]:
    agente = "CalendarioEventosV2.5.1/1.0 analisis-inversion-local"
...
def _impacto_visual(
    porcentaje: float | None, futuro: bool = False
) -> tuple[str, str, str, int]:
    if futuro:
        return "Pendiente", "#5f6368", "El sentido del impacto todavía es desconocido", 400
    if porcentaje is None:
        return "Sin datos", "#5f6368", "No hay cotización suficiente", 400
    peso = 800 if abs(porcentaje) > UMBRAL_MOVIMIENTO_FUERTE_PCT else 400
    if porcentaje > 0:
        return f"↑ {porcentaje:+.2f} %", "#188038", "Subida", peso
    if porcentaje < 0:
        return f"↓ {porcentaje:+.2f} %", "#d93025", "Bajada", peso
    return f"→ {porcentaje:+.2f} %", "#5f6368", "Sin variación", 400


def _patron_comportamiento_html(ticker: str) -> str:
    if ticker != "NVDA":
        return ""
    return """
        <div class="patron-comportamiento">
          <h4>PATRÓN COMPORTAMIENTO</h4>
          <p><b>Patrón predominante observado:</b> momentum catalizado por eventos, que con
          frecuencia adopta la secuencia <b>impulso → consolidación o pullback → posible
          continuación</b>. No es un patrón fijo ni garantiza que aparezca el segundo impulso.</p>
          <p>La interpretación combina tres rasgos documentados: deriva posterior a sorpresas de
          resultados (PEAD), sensibilidad a revisiones de expectativas y agrupamiento de
          volatilidad. En NVDA los resultados, la guía, las restricciones de exportación y las
          noticias sobre competencia o demanda pueden invalidar de inmediato el pullback.</p>
          <p class="fuentes-patron">Referencias:
          <a href="https://doi.org/10.1016/j.jbef.2020.100446" target="_blank" rel="noopener noreferrer">revisión académica sobre PEAD</a> ·
          <a href="https://pages.stern.nyu.edu/~adamodar/pdfiles/blog/Nvidia2024.pdf" target="_blank" rel="noopener noreferrer">informe de A. Damodaran sobre NVIDIA y momentum</a> ·
          <a href="https://vlab.stern.nyu.edu/volatility/VOL.NVDA%3AUS-HL.AMEM" target="_blank" rel="noopener noreferrer">NYU V-Lab: volatilidad de NVDA</a>.
          Es una caracterización estadística, no una señal de compra o venta.</p>
        </div>"""
...
def _bloque_html(fila: dict[str, Any]) -> str:
    ...
    for evento in fila["historicos"]:
        ...
        impacto, color, direccion, peso = _impacto_visual(evento["impacto"])
        historicos_html.append(f"""
          <tr>
            ...
            <td class="impacto" style="color:{color};font-weight:{peso}" title="{direccion}">{escape(impacto)}</td>
            <td class="fuente">{_enlaces_noticias(evento['noticias'])}</td>
          </tr>""")
    if not historicos_html:
        historicos_html.append(
            '<tr><td colspan="5" class="vacio">No hubo sesiones con variación igual o superior al 5 % '
    ...
    return f"""
      <section class="bloque" title="{escape(aviso)}">
        ...
        {_patron_comportamiento_html(fila['ticker'])}
...
<title>Calendario dinámico de eventos V2.5.1</title><style>
 ...
 .impacto {{ width:100px;white-space:nowrap;font-size:13px;text-align:center }}
 ...
 .patron-comportamiento {{ background:#f8fbff;border-left:4px solid #174ea6;
                           padding:8px 10px;margin:6px 0 12px;font-size:11px;line-height:1.4 }}
 .patron-comportamiento p {{ margin:5px 0 }} .fuentes-patron {{ color:#5f6368 }}
 ...
 de resultados y las sesiones cuya variación cierre a cierre fue igual o superior al 5 %. Los
 movimientos entre 5 % y 7 % se muestran sin negrita; los superiores al 7 %, en negrita. Los eventos futuros
...
def generar_calendario_eventos(
    ...
    for ticker in tickers:
        ...
        for fecha_movimiento, impacto in grandes:
            ...
            historicos.append({
                ...
                "tipo": (
                    "Movimiento diario superior al 7 %"
                    if abs(impacto) > UMBRAL_MOVIMIENTO_FUERTE_PCT
                    else "Movimiento diario entre el 5 % y el 7 %"
                ),
                "resumen": resumen,
                "impacto": impacto,
                "noticias": noticias,
    ...
    if mostrar:
        display(HTML(html))
        print("Informe V2.5.1 guardado en:", ARCHIVO_HTML.resolve())
